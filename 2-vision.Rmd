...
Why does the way in which we approach statistics for data science need reframing? Statistical Science tends to begin with a framework of stochastic assumptions, describing properties and methods that act as a solution to a problem -- a "framework-first" approach. But Statistics for Data Science requires a "problem-first" approach, focussing on appropriately constructing a framework from a given real-world problem. These are not mindsets that are translatable, because real problems don't neatly satisfy assumptions. Instead, Statistics for Data science involves a critical evaluation as to how realistic and how useful assumptions are, as well as what output is suitable for the problem type. This isn't assumption-_checking_, it's assumption-_building_.
